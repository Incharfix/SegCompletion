...
#备注： 评估 ShapeNet 上的性能
# 程序会在'test-examples'中生成txt文件

'''
--dataroot  ./dataset/shapenet_part/shapenetcore_partanno_segmentation_benchmark_v0/
--class_choice Bag
--netG Checkpoint/point_netG46.pth
'''
...
parser.add_argument('--dataroot',  default='dataset/train', help='path to dataset')  # 加载数据集的路径
...
parser.add_argument('--batchSize', type=int, default=1, help='input batch size')          # 这个一般不改
parser.add_argument('--pnum', type=int, default=2048, help='the point number of a sample') # 这个一般不改
...
errG_min = 100
n = 0
#np.loadtxt('test_example/crop1.csv',delimiter=',')
...
#    if a!=b:
        ...
        np.savetxt('test_example/crop'+str(n)+'.csv', np_crop, fmt = "%f,%f,%f")
        np.savetxt('test_example/fake'+str(n)+'.csv', np_fake, fmt = "%f,%f,%f")
        np.savetxt('test_example/real'+str(n)+'.csv', np_real, fmt = "%f,%f,%f")
