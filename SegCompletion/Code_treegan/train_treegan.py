...
def InitTreeGan(args,device):
    ...
    if args.krepul_loss:  # PU-net      default=False
        krepul_loss = kNNRepulsionLoss(k=args.krepul_k, n_seeds=args.krepul_n_seeds,
                                            h=args.krepul_h)

    if args.knn_loss:  # PatchVariance   default=True
        knn_loss = kNNLoss(k=args.knn_k,
                                n_seeds=args.knn_n_seeds)  # args.knn_k default=30   #args.knn_n_seeds default=100
    ######################################################################
